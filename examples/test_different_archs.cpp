...
template <typename InputType, typename MiddleType, typename OutputType>
class FixedPointCNN
{
public:
...
void train (void)
{
    ...
    auto cnn = FixedPointCNN<FixedPoint<I1, I2>, FixedPoint<I3, I4>, FixedPoint<I5, I6>>(inputDimensions);
	cnn.train(settings, trainingData, optimizer);

    // printing can be affected by a race condition - not experienced, so not handled ;-)
	printf("LAYERS: %i %i %i %i %i %i\nRESULT: %f\n", I1, I2, I3, I4, I5, I6, cnn.validate(validationData));
}
